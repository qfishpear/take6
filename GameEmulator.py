...
from YXJAgent import EasiestAgent
import numpy as np
...
class GameEmulator(object):
    ...
    def emulate(self, num_round = 1):
        ...
        for i in range(num_round):
            ...
            cnt_win[np.argmax(game_env["scores"])] += 1
        print cnt_win

    def emulate_66(self, num_round = 1):
        cnt_win = [0, ] * len(self.agent_list)
        for i in range(num_round):
            total_score = [0,] * len(self.agent_list)
            while max(total_score) < 66:
                game_env = self.generate_env(1)
                for j in range(num_agent_init_card):
                    game_env["agent_id"] = 0
                    action0 = self.agent_list[0].policy(game_env)
                    game_env, punishments = self.execute_action(game_env, action0)
                #print game_env["scores"]
                total_score = [total_score[j] + game_env["scores"][j] for j in range(len(self.agent_list))]
            print total_score
            cnt_win[np.argmax(total_score)] += 1
        print cnt_win
